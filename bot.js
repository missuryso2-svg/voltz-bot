...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isButton()) {
    ...
    // VALIDAÇÃO
    if (interaction.customId === 'validacao') {

      try {

        const memberRole = interaction.guild.roles.cache.find(
          r => r.name.trim() === ROLE_MEMBER.trim()
        );

        if (!memberRole) {

          return interaction.reply({
            content: `❌ Cargo "${ROLE_MEMBER}" não encontrado.`,
            ephemeral: true
          });
        ...
        return interaction.reply({
          content: `✅ Cargo ${memberRole} recebido com sucesso.`,
          ephemeral: true
        });
        ...
        return interaction.reply({
          content: '❌ Ocorreu um erro ao validar você.',
          ephemeral: true
        });
